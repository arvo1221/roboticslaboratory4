...
// Output : None
//////////////////////////////////////////////////////////////////
void InitTimer0(){//0번 타이머 overflow interrupt 마다 프로그램 실행
	
	TCCR0 = 0x04;//normal, 64분주비
	TIMSK = 0x01;//0번 timer overflow interrupt enable
}
...
void InitTimer1(){
	...
	//TO DO
	TCCR1A = 0xE2;//0b11100010;//set OC1A on compare match(set output to high level) clear OC1A CON COMPARE MATCH(SET OUTPUT TO LOW LEVEL)
	TCCR1B = 0x11;//0b00010001;PWM Phase Correct TOP:ICRn UPDATE TOP TOVn Flag set on bbottom
...
void InitUart0(){
	...
	//Uart
	UCSR0A = 0x00;
	UCSR0B = 0x98;//0b1001 1000 RX Complete Interrupt Enable RX,TX Enable
	UCSR0C = 0x06;//0b0000 0110 data 8bit
...
void InitUart1(){
	...
	UCSR1A = 0x00;
	UCSR1B = 0x18;//Enable RX,TX Enable
	UCSR1C = 0x06;//data 8bit
...
// Output : None
//////////////////////////////////////////////////////////////////
void TransNumUart0(int32_t num){
...
void TransdoubleNum0(double complex){
	
	int32_t num = (int)(complex * 100);
	//TO DO
	if(num < 0){
		TransUart0('-');
		num *= -1;
